...
async function handleRequest(
  ...
  if (req.method === "POST" && path === "/register") {
    ...
    res.end(
      JSON.stringify({
        ...
        redirect_uris: body
          ? ((JSON.parse(body) as { redirect_uris?: string[] })
              .redirect_uris ?? [authorizeUrl("/callback")])
  ...
  try {
    const body: unknown =
      req.method === "POST" ? JSON.parse(await readBody(req)) : undefined;

    // Example server uses the low-level `Server` API on purpose.
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    const srv = new Server(
      { name: "demo-http-oauth", version: "1.0.0" },
      { capabilities: { tools: {} } },
    );
...
app.listen(PORT, () => {
  console.error(`OAuth demo MCP server listening on port ${String(PORT)}`);
});
