...
const basePath =
  process.argv.find((a) => a.startsWith("--base-path="))?.split("=")[1] ??
  "/tmp/demo";
...
const server = new Server(
  { name: "demo-params", version: "1.0.0" },
  { capabilities: { tools: {} } },
);
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          dir: {
            type: "string",
            description: "Relative directory path from base",
          },
          pattern: {
            type: "string",
            description: "Glob-style filter (e.g. *.txt, data-*)",
          },
          recursive: {
            type: "boolean",
  ...
  try {
    switch (tool) {
      case "read": {
        ...
        const maxSize = (opts.maxSize as number | undefined) ?? 65536;
      ...
      case "write": {
        ...
        const append = (opts.append as boolean | undefined) ?? false;
          ...
          content: [
            {
              type: "text",
              text: prefix(
                `Written ${String(content.length)} bytes to ${relPath}`,
              ),
            },
          ],
        };
      ...
      case "list": {
        ...
        if (!fullDir.startsWith(basePath))
          throw new Error("Path traversal denied");
        ...
        return {
          content: [
            { type: "text", text: prefix(JSON.stringify(filtered, null, 2)) },
          ],
        };
      }
      case "stats": {
