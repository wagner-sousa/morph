...
describe("end-to-end tool routing over stdio", () => {
  ...
  beforeAll(async () => {
    ...
    const toolsByMcp = new Map(
      [...registry.getConnectedClients().keys()].map((n) => [
        n,
        registry.getTools(n),
      ]),
    );
  ...
  it("namespaces conflicting tools across MCPs", () => {
    ...
    expect(router.resolve("alpha_echo")).toEqual({
      mcpName: "alpha",
      originalName: "echo",
    });
  });
  ...
  it("converts a large_json response to TOON with savings", async () => {
    const converter = new ToonConverter(
      validateConfig({ mcpServers: [] }).toon,
    );
