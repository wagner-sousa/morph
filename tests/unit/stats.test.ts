...
describe('estimateTokens', () => {
  ...
  it('returns 1 for single char', () => expect(estimateTokens('a')).toBe(1));
  it('returns 1 for 4 chars', () => expect(estimateTokens('abcd')).toBe(1));
  it('returns 2 for 5 chars', () => expect(estimateTokens('abcde')).toBe(2));
  it('handles whitespace', () => expect(estimateTokens('  ')).toBe(1));
  it('handles non-ASCII', () => expect(estimateTokens('áéíóú')).toBe(2));
});
