...
describe('healthcheck.ts', () => {
  it('exists and contains expected code', () => {
    const healthcheckPath = resolve(import.meta.dirname, '../../src/healthcheck.ts');
...
describe('retry', () => {
  it('retries on failure', async () => {
    const { retry, RetryAbortedError } = await import('../../src/utils/retry.js');
    let calls = 0;
    const fn = vi.fn(async () => { calls++; if (calls < 3) throw new Error('fail'); return 'ok'; });
  ...
  it('throws RetryAbortedError when all retries exhausted', async () => {
    ...
    await expect(retry(fn, { retries: 1, factor: 0 })).rejects.toThrow('always fail');
  });
