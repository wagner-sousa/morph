...
describe('Metrics', () => {
  ...
  beforeEach(() => { metrics = new Metrics(); });
  afterEach(() => { metrics.removeAllListeners(); });
  ...
  it('records a successful call', () => {
    metrics.record({ mcpName: 'fs', toolName: 'read', durationMs: 100, tokensSaved: 50, success: true }, 25);
    const snap = metrics.snapshot();
  ...
  it('records multiple calls and aggregates', () => {
    metrics.record({ mcpName: 'fs', toolName: 'read', durationMs: 100, tokensSaved: 50, success: true }, 25);
    metrics.record({ mcpName: 'fs', toolName: 'write', durationMs: 200, tokensSaved: 30, success: true }, 15);
    metrics.record({ mcpName: 'db', toolName: 'query', durationMs: 50, tokensSaved: 0, success: false }, 0);
    const snap = metrics.snapshot();
  ...
  it('groups by MCP in snapshot', () => {
    metrics.record({ mcpName: 'fs', toolName: 'read', durationMs: 100, tokensSaved: 50, success: true }, 25);
    metrics.record({ mcpName: 'db', toolName: 'write', durationMs: 50, tokensSaved: 10, success: true }, 10);
    const snap = metrics.snapshot();
  ...
  it('emits update event on record', () => {
    ...
    metrics.record({ mcpName: 'fs', toolName: 'read', durationMs: 10, tokensSaved: 5, success: true }, 10);
