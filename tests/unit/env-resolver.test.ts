...
describe("resolveEnvVars", () => {
  ...
  it("throws EnvResolutionError listing all missing vars in strict mode", () => {
    ...
    ).toThrowError(EnvResolutionError);
    try {
      resolveEnvVars("${MISSING_A}-${MISSING_B}", { env });
    } catch (err) {
      expect((err as EnvResolutionError).missing).toEqual([
        "MISSING_A",
        "MISSING_B",
      ]);
    }
  ...
  it("leaves placeholders untouched in non-strict mode", () => {
    expect(resolveEnvVars("${MISSING}", { env, strict: false })).toBe(
      "${MISSING}",
    );
  });

  it("does not mutate non-string primitives", () => {
    expect(resolveEnvVars({ n: 5, b: true, x: null }, { env })).toEqual({
      n: 5,
      b: true,
      x: null,
    });
