...
describe("MorphOAuthProvider", () => {
  it("redirectUrl formats correctly", () => {
    ...
    expect(p.redirectUrl).toBe(
      "http://localhost:3101/api/mcps/test-mcp/oauth/callback",
    );
  });

  it("clientMetadata has redirect_uris", () => {
    const p = createProvider();
    expect(p.clientMetadata.redirect_uris).toContain(
      "http://localhost:3101/api/mcps/test-mcp/oauth/callback",
    );
  ...
  it("saveClientInformation delegates to store", async () => {
    ...
    expect(
      (p as unknown as { store: { set: ReturnType<typeof vi.fn> } }).store.set,
    ).toHaveBeenCalled();
  });
  ...
  it("redirectToAuthorization stores URL", async () => {
    ...
    expect(
      (p as unknown as { store: { set: ReturnType<typeof vi.fn> } }).store.set,
    ).toHaveBeenCalledWith("test-mcp", { authorizationUrl: "http://auth/url" });
  });
  ...
  it("invalidateCredentials calls store.delete for all", async () => {
    ...
    expect(
      (p as unknown as { store: { delete: ReturnType<typeof vi.fn> } }).store
        .delete,
    ).toHaveBeenCalledWith("test-mcp");
  });
});
