...
describe("config loader", () => {
  ...
  it("infers http/sse transports from the entry fields", () => {
    ...
    const byName = Object.fromEntries(
      cfg.mcpServers.map((s) => [s.name, s.transport.type]),
    );
  ...
  it("resolves env vars when parsing JSON text", () => {
    const [m, mcp] = texts({
      k: { type: "http", url: "https://x/mcp", apiKey: "${MY_KEY}" },
    });
  ...
  it("throws on invalid JSON", () => {
    expect(() => parseConfig("{ not json", undefined)).toThrowError(
      ConfigError,
    );
  ...
  it("still requires env vars for enabled MCP servers", () => {
    const [m, mcp] = texts({
      on: { enabled: true, command: "${MISSING_CMD}" },
    });
...
describe("dedicated MORPH_* env overrides", () => {
  ...
  it("env wins over the JSON value, JSON wins over default", () => {
    const [m, mcp] = texts(
      {},
      { webUi: { port: 5000 }, morph: { logLevel: "warn" } },
    );
  ...
  it("accepts common boolean spellings", () => {
    ...
    for (const v of ["true", "1", "yes", "on"]) {
      expect(
        parseConfig(m, mcp, { env: { MORPH_WEB_ENABLED: v } }).webUi.enabled,
      ).toBe(true);
    }
  ...
  it("ignores empty-string env vars", () => {
    ...
    expect(
      parseConfig(m, mcp, { env: { MORPH_WEB_PORT: "" } }).webUi.port,
    ).toBe(5000);
  });
  ...
  it("rejects invalid boolean, integer and enum env values", () => {
    const [m, mcp] = texts({});
    expect(() =>
      parseConfig(m, mcp, { env: { MORPH_WEB_ENABLED: "maybe" } }),
    ).toThrowError(ConfigError);
    expect(() =>
      parseConfig(m, mcp, { env: { MORPH_WEB_PORT: "abc" } }),
    ).toThrowError(ConfigError);
    expect(() =>
      parseConfig(m, mcp, { env: { MORPH_TOON_DELIMITER: "xml" } }),
    ).toThrowError(ConfigError);
  });
