...
describe('HealthChecker', () => {
  ...
  beforeEach(() => {
    ...
    checker = new HealthChecker(registry, { intervalMs: 5000, timeoutMs: 1000, maxRetries: 3 }, logger);
  });
  ...
  it('calls refreshTools on connected clients', async () => {
    ...
    registry.getConnectedClients = vi.fn().mockReturnValue(
      new Map([['fs', { getStatus: () => 'connected' }]]),
    );
