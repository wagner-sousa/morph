...
describe('LogStore', () => {
  it('auto-assigns sequential IDs when no id is provided', () => {
    ...
    const e1 = store.append({ mcpName: 'fs', toolName: 'read', level: 'info', message: 'a' });
    const e2 = store.append({ mcpName: 'fs', toolName: 'write', level: 'info', message: 'b' });
    expect(e1.id).toBe(1);
    expect(e2.id).toBe(2);
  });
  ...
  it('accepts a custom id via the id field', () => {
    const store = new LogStore();
    const entry = store.append({ id: 42, mcpName: 'fs', toolName: 'read', level: 'info', message: 'custom' });
    expect(entry.id).toBe(42);
  });

  it('uses the provided id and does not increment nextId for it', () => {
    const store = new LogStore();
    store.append({ id: 100, mcpName: 'fs', toolName: 'read', level: 'info', message: 'custom' });
    // Next auto-assigned id should still start from 1
    const e2 = store.append({ mcpName: 'fs', toolName: 'write', level: 'info', message: 'after custom' });
    expect(e2.id).toBe(1);
  });

  it('query returns the entry with the correct id', () => {
    const store = new LogStore();
    store.append({ id: 99, mcpName: 'fs', toolName: 'read', level: 'info', message: 'data' });
