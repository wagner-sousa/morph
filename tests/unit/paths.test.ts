...
describe("resolvePaths", () => {
  ...
  it("lets MORPH_LOG_DIR override the derived log dir", () => {
    const p = resolvePaths({
      env: { MORPH_DATA_DIR: dir, MORPH_LOG_DIR: "/var/log/morph" },
    });
  ...
  it("honors MORPH_CONFIG env over the data dir", () => {
    ...
    const p = resolvePaths({
      env: { MORPH_DATA_DIR: dir, MORPH_CONFIG: custom },
    });
    expect(p.configPath).toBe(resolve(custom));
  });
});
...
describe("resolveMcpConfigPath", () => {
  it("derives a sibling from the config name suffix", () => {
    ...
    expect(resolveMcpConfigPath("/x/morph.demo.json")).toBe(
      resolve("/x/.mcp.demo.json"),
    );
  });

  it("honors an explicit override", () => {
    expect(resolveMcpConfigPath("/x/morph.json", "/y/servers.json")).toBe(
      resolve("/y/servers.json"),
    );
  });
});
