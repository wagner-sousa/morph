...
function fetchPost(url: string, body: string, headers?: Record<string, string>): Promise<{ status: number; body: string }> {
...
async function stdioRequest(proc: ChildProcess, req: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.stdout?.on('data', () => clearTimeout(timeout));
  });
}
...
async function waitForPort(port: number, timeoutMs = 5000): Promise<void> {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      await fetchPost(`http://localhost:${port}/mcp`, jsonRpcRequest('initialize', { protocolVersion: '2024-11-05' }));
      return;
    } catch {
...
describe('demo-mcp-server (STDIO)', () => {
  ...
  it('responds to tools/list and tools/call', async () => {
    ...
    const initRes = await stdioRequest(proc, jsonRpcRequest('initialize', { protocolVersion: '2024-11-05' }));
    ...
    const callRes = await stdioRequest(proc, jsonRpcRequest('tools/call', { name: 'ping' }));
...
describe('http-mcp-server', () => {
  ...
  it('responds to tools/list', async () => {
    const initRes = await fetchPost('http://localhost:3200/mcp', jsonRpcRequest('initialize', { protocolVersion: '2024-11-05' }));
  ...
  it('responds to tools/call(ping) with pong', async () => {
    const res = await fetchPost('http://localhost:3200/mcp', jsonRpcRequest('tools/call', { name: 'ping' }));
...
describe('sse-mcp-server', () => {
  ...
  it('responds to tools/call(ping)', async () => {
    const res = await fetchPost('http://localhost:3201/mcp', jsonRpcRequest('tools/call', { name: 'ping' }));
    const body = JSON.parse(res.body);
    expect(body.result?.content?.[0]?.text).toBe('pong');
  });
});
...
describe('oauth-mcp-server', () => {
  ...
  it('rejects requests without token with 401', async () => {
    const res = await fetchPost('http://localhost:3202/mcp', jsonRpcRequest('tools/call', { name: 'ping' }));
  ...
  it('accepts requests with Bearer demo-token', async () => {
    const res = await fetchPost('http://localhost:3202/mcp', jsonRpcRequest('tools/call', { name: 'ping' }), { Authorization: 'Bearer demo-token' });
  ...
  it('exposes OAuth metadata at /.well-known/oauth-authorization-server', async () => {
    const res = await fetchPost('http://localhost:3202/.well-known/oauth-authorization-server', '');
    // GET request
    const u = new URL('http://localhost:3202/.well-known/oauth-authorization-server');
...
describe('param-mcp-server', () => {
  ...
  it('lists tools with inputSchema parameters', async () => {
    proc = spawnStdio('param-mcp-server.js', ['--base-path', '/tmp/demo-test'], { DEMO_MODE: 'true' });
    await stdioRequest(proc, jsonRpcRequest('initialize', { protocolVersion: '2024-11-05' }));
    const listRes = await stdioRequest(proc, jsonRpcRequest('tools/list'));
  ...
  it('prepends [DEMO] prefix when DEMO_MODE=true', async () => {
    ...
    await stdioRequest(proc, jsonRpcRequest('initialize', { protocolVersion: '2024-11-05' }));
    // write a test file
    await stdioRequest(proc, jsonRpcRequest('tools/call', { name: 'write', arguments: { path: 'hello.txt', content: 'world' } }));
    const readRes = await stdioRequest(proc, jsonRpcRequest('tools/call', { name: 'read', arguments: { path: 'hello.txt' } }));
