...
async function sseSession(url: string): Promise<{ sessionId: string; close: () => void }> {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.setTimeout(5000, () => { req.destroy(); reject(new Error('SSE session timeout')); });
  });
}
...
describe('sse-mcp-server', () => {
  ...
  it('starts and accepts SSE connections', async () => {
    ...
    const ack = await fetchPost(`http://localhost:3201/mcp?sessionId=${session.sessionId}`, jsonRpcRequest('notifications/initialized'));
...
describe('oauth-mcp-server', () => {
  ...
  it('exposes OAuth metadata at /.well-known/oauth-authorization-server', async () => {
    const res = await fetchGet('http://localhost:3202/.well-known/oauth-authorization-server');
    expect(res.status).toBe(200);
