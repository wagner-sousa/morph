...
import { MCPDefinitionSchema, MorphConfigSchema, StdioTransportSchema, HttpTransportSchema, SseTransportSchema } from '../../src/config/schema.js';
...
describe('POST /api/mcps — schema validation (contract tests)', () => {
  it('accepts a valid stdio definition', () => {
    const payload = {
      name: 'my-server',
      enabled: true,
      transport: { type: 'stdio', command: 'npx', args: ['-y', 'some-package'] },
    };
    const parsed = MCPDefinitionSchema.parse(payload);
    expect(parsed.name).toBe('my-server');
    expect(parsed.transport).toEqual({ type: 'stdio', command: 'npx', args: ['-y', 'some-package'] });
  });
  ...
  it('accepts a valid http definition', () => {
    ...
    const parsed = MCPDefinitionSchema.parse(payload);
    expect(parsed.transport).toEqual({ type: 'http', url: 'http://localhost:8000/mcp' });
  });
  ...
  it('accepts a valid sse definition', () => {
    ...
    const parsed = MCPDefinitionSchema.parse(payload);
    expect(parsed.transport).toEqual({ type: 'sse', url: 'http://localhost:8000/events' });
  });
  ...
  it('rejects duplicate names via superRefine', () => {
    const cfg = {
      mcpServers: [
        { name: 'dup', enabled: true, transport: { type: 'stdio', command: 'npx' } },
        { name: 'dup', enabled: true, transport: { type: 'stdio', command: 'npx' } },
      ],
    };
