...
describe('SseMCPClient', () => {
  ...
  it('extends BaseMCPClient', () => {
    const client = new SseMCPClient('test', { type: 'sse', url: 'http://localhost:3201/sse' }, { logger });
    expect(client).toBeInstanceOf(BaseMCPClient);
  });

  it('stores headers', () => {
    const client = new SseMCPClient('test', { type: 'sse', url: 'http://localhost:3201/sse', headers: { Authorization: 'Bearer x' } }, { logger });
    expect(client).toBeDefined();
  });

  it('stores reconnectIntervalMs', () => {
    const client = new SseMCPClient('test', { type: 'sse', url: 'http://localhost:3201/sse', reconnectIntervalMs: 5000 }, { logger });
    expect(client).toBeDefined();
  });
