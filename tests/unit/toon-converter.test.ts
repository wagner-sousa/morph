...
describe('ToonConverter', () => {
  ...
  it('converts a large uniform JSON array and reports savings', () => {
    const rows = Array.from({ length: 30 }, (_, i) => ({ id: i, name: `n${i}`, active: true }));
    const { result, converted, savings } = converter.convertResult(textResult(JSON.stringify(rows)));
    expect(converted).toBe(true);
    expect(savings!.percent).toBeGreaterThan(0);
    const item = result.content[0] as { text: string; _meta?: Record<string, unknown> };
  ...
  it('leaves non-text content alone', () => {
    const res: CallToolResult = { content: [{ type: 'image', data: 'abc', mimeType: 'image/png' }] };
