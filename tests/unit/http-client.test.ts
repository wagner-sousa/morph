...
describe('HttpMCPClient', () => {
  ...
  it('extends BaseMCPClient', () => {
    const client = new HttpMCPClient('test', { type: 'http', url: 'http://localhost:3200/mcp' }, { logger });
    expect(client).toBeInstanceOf(BaseMCPClient);
  });

  it('stores apiKey in headers', () => {
    const client = new HttpMCPClient('test', { type: 'http', url: 'http://localhost:3200/mcp', apiKey: 'demo-token' }, { logger });
    expect(client).toBeDefined();
  });

  it('stores custom headers', () => {
    const client = new HttpMCPClient('test', { type: 'http', url: 'http://localhost:3200/mcp', headers: { 'X-Custom': 'val' } }, { logger });
    expect(client).toBeDefined();
  });
