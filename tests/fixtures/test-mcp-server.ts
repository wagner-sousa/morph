...
// Test fixture uses the low-level `Server` API on purpose.
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: "delay",
      description: "Delay then echo",
      inputSchema: { type: "object" },
    },
    {
      name: "large_json",
      description: "Large uniform JSON",
      inputSchema: { type: "object" },
    },
  ],
}));
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  switch (name) {
    ...
    case "delay": {
      const ms = (args as { ms?: number } | undefined)?.ms ?? 10;
