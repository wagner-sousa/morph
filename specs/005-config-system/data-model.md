...
contract) and [src/config/types.ts](../../src/config/types.ts) (inferred types).
...
> update this view, never hand-edit the generated JSON.
...
of the codebase consumes (`schema.ts:161`).
...
### Transport (discriminated union by `type`) — `TransportSchema` (`schema.ts:46`)

The internal, explicit transport. One of:

| Variant | Schema | Required | Optional |
|---------|--------|----------|----------|
| `stdio` | `StdioTransportSchema` (`:23`) | `command` (non-empty) | `args` (default `[]`), `env`, `cwd`, `timeoutMs` (int >0) |
| `http` | `HttpTransportSchema` (`:32`) | `url` (valid URL) | `headers`, `apiKey` |
| `sse` | `SseTransportSchema` (`:39`) | `url` (valid URL) | `headers`, `reconnectIntervalMs` (int >0) |

### MCP server entry (on-disk, flat) — `McpServerEntrySchema` (`schema.ts:68`)

The Claude-style flat shape written in `.mcp.json`. The transport is **inferred** from the
fields. `superRefine` (`schema.ts:86`) enforces:
...
### MCP definition (internal, post-merge) — `MCPDefinitionSchema` (`schema.ts:53`)
...
- `toMcpDefinitions(servers)` (`schema.ts:219`) — `.mcp.json` map → `MCPDefinition[]`.
- `fromMcpDefinitions(defs)` (`schema.ts:185`) — `MCPDefinition[]` → `.mcp.json` map.

### Settings sub-objects (all with defaults)

| Entity | Schema | Key fields (with defaults / ranges) |
|--------|--------|-------------------------------------|
| Morph settings | `MorphSettingsSchema` (`:135`) | `version` (`"1.0"`), `logLevel` (`info`), `allowConflicts` (`false`), `toolPrefix` (`""`, template where `{name}` → MCP name) |
| TOON options | `ToonOptionsSchema` (`:102`) | `autoConvert` (`true`), `delimiter` (`comma`), `indent` (int 0–8, `2`), `flattenDepth` (int ≥0, `4`), `threshold` (int ≥0, `100`) |
| Web UI | `WebUiSchema` (`:112`) | `enabled` (`true`), `host` (`0.0.0.0`), `port` (int 1–65535, `3100`), `publicUrl?` (URL), `auth?` (`{username, passwordHash}`) |
| Health | `HealthSchema` (`:127`) | `intervalMs` (int >0, `30000`), `timeoutMs` (int >0, `5000`), `maxRetries` (int ≥0, `3`) |

### Merged config — `MorphConfigSchema` (`schema.ts:161`)

`{ morph, mcpServers: MCPDefinition[], toon, webUi, health }`. Its `superRefine`
(`schema.ts:170`) rejects **duplicate MCP names**, reporting the offending
...
- Loading + `${ENV}` resolution: `src/config/loader.ts`, `src/utils/env.ts`.
